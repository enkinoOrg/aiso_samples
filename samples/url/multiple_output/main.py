...
def task(image: np.ndarray) -> List[np.ndarray]:
...
def index():
    ...
    headers = aiso_request["header"]
    upload_urls = aiso_request["upload_url"]
    ...
    # task 실행
    result_imgs = task(cv2_img)

    # 여러개의 결과값 업로드
    for result_img, header, upload_url in zip(result_imgs, headers, upload_urls):
        # 결과 전 치리 
